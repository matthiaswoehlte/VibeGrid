# CC #1 Prompt — Plan 9c.2: Playback-Clock-Fallback + Metronom (Rev. 4)
...
**B1 — Metronom-Toggle nicht in der `audio`-Slice ablegen.**
`HistoryEntry` snapshottet die ganze `audio`-Slice (`history-types.ts:18-21`),
`undo()/redo()` restaurieren wholesale. Läge `metronomeEnabled` in `audio`,
setzte jedes Undo/Redo den Toggle zurück. **Fix: Toggle in die `ui`-Slice
legen** — `ui` ist nicht Teil des Undo-Snapshots (`history-types.ts:14`),
also kein Eingriff in `history-actions.ts` (ESLint-Whitelist bleibt
unberührt). Persistenz: `persist-shape.ts:30` (heute `ui: { zoom }`) um
`metronomeEnabled` erweitern.
...
**DL3 — Store-Version:** `metronomeEnabled` ist ein neues persistiertes Feld
in `ui`. Entscheidung: **kein Versions-Bump**, Default-Merge (`false`) für
Altprojekte (fehlendes `ui.metronomeEnabled` → `false`). Falls der
Hydration-Pfad strikt validiert statt zu mergen → v7→v8-Migration mit
`metronomeEnabled: false`. In Schritt 0 am Hydration-Pfad bestätigen.
...
5. ⚠️ **Beat-Quelle fürs Metronom:** `loop.ts:309/:531` — Beat-Formel
   `(time - grid.offsetMs/1000) * bpm/60`. Metronom nutzt **dieselbe Formel,
   aber vorwärts projiziert** (künftige Beat-Zeiten auf der AudioContext-
   Achse), plus Mapping `currentTime` → AudioContext-Zeit. Das ist neue
   Logik, kein Read der Momentan-`beats`.
6. ✅ **Taktart existiert:** `BeatGrid.beatsPerBar` (`types.ts:6`, Default 4
   in `DEFAULT_BEAT_GRID`, von Detection gesetzt `engine.ts:246`). Takt-1
   via `beatIndex % grid.beatsPerBar === 0`. `grid.offsetMs` für die
   Projektion bestätigen.
...
| Store + `persist-shape.ts:30` | MODIFY — `metronomeEnabled` in `ui`-Slice, persistent |
...
19. Toggle in `ui`-Slice: überlebt Reload (persistent); Undo UND Redo lassen
    `metronomeEnabled` unverändert (liegt außerhalb des `audio`-Snapshots)
...
- [ ] Metronom-Toggle in `ui`-Slice (nicht `audio`) → Undo/Redo unberührt, `history-actions.ts` nicht angefasst (Test 19)
- [ ] B2: Fallback-Play ruft `resume()` (Test 12b)
- [ ] Metronom: `beatsPerBar` + `offsetMs` in der Beat-Projektion (W), BPM aus `audio.grid.bpm`

---

Rev. 4 — Clock-Fallback (AudioContext, ternär, Gate 0.0) + Metronom in einer
