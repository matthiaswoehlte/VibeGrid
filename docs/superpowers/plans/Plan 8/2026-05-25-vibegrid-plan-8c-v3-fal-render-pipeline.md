# CC #1 Prompt — Schreibe Plan 8c: fal.ai Render-Pipeline (Rev. 3)

> **Revision 3 — 2026-05-25**
> Rev. 2: Alle 14 Punkte aus dem ersten Architekt-Review (B1–B3, W1–W6, D1–D5).
> Rev. 3: Alle 4 Wackler aus dem zweiten Architekt-Review (N1–N4) eingearbeitet.
> Markiert mit `[Fix Bx]` / `[Fix Wx]` / `[Fix Dx]` / `[Fix Nx]`.
...
### [Fix N3] SSE vs. Polling — wo welches Pattern

**Phase 1 (Image + Voice): ebenfalls Queue-basiert.**
Begründung: Bei 20 Szenen × max-3-parallel = ~7 Batches × 15 s = 105 s → über
Vercel-Hobby-Timeout (60 s). SSE hält die Lambda trotzdem am Leben. Konsequenz:
zuverlässige Timeouts, inkonsistente Teilergebnisse. Queue ist konsistenter:

```
POST /api/sceneflow/stories/[id]/generate-images-and-voices
  → enqueued alle FLUX-Jobs via fal.queue.submit, TTS direkt (sync, <10 s)
  → TTS-Ergebnisse sofort → R2 → audio_url gesetzt
  → FLUX request_ids in fal_request_ids.image gespeichert
  → Response: { enqueued: number }  (schnell, <5 s)

GET /api/sceneflow/scenes/[id]/status  (gemeinsamer Endpoint für Phase 1 + 2)
  → prüft alle fal_request_ids der Szene
  → wenn COMPLETED: result holen → R2-Upload → image_url / video_url setzen
```

**Phase 2 (Video):** Queue-Submit-and-Poll-Pattern (wie bisher):
```
...
**Vorteil:** Konsistente Architektur für beide Phasen. Browser-Tab kann
jederzeit geschlossen werden. Beim Wiederöffnen zeigt Polling sofort den
aktuellen Stand aller Jobs.
...
| Dialog-Szene: Charakter mit `voice_provider === 'azure'` | 🔴 "Azure TTS nicht implementiert — bitte auf Edge oder ElevenLabs wechseln (v0.2)" |
...
2. Bild-Generierung aller Szenen via FLUX — alle parallel via
   `fal.queue.submit` (fal.ai handelt Throttling selbst, kein
   clientseitiges max-3-Limit nötig); `request_ids` landen in
   `fal_request_ids.image`.
3. TTS-Ergebnisse sofort → R2 → `audio_url`. FLUX-Ergebnisse über
   Status-Polling → R2 → `image_url`.
...
Schritt B: fal.queue.submit(sync-lipsync/v3 ODER MuseTalk)
...
**[Fix N1] Wer startet Schritt B?**
Option (b): **der Status-Endpoint** enqueued Schritt B automatisch, sobald er
`neutral_video_url` setzt. Kein Client-Round-Trip nötig:

```typescript
// GET /api/sceneflow/scenes/[id]/status — Logik
if (neutralVideoCompleted && !scene.fal_request_ids?.lipsync) {
  // neutral_video_url setzen + sofort Schritt B enqueuen
  const { request_id } = await fal.queue.submit(lipSyncModel, { ... });
  await updateScene(sceneId, {
    neutral_video_url: r2NeutralUrl,
    fal_request_ids: { ...existing, lipsync: request_id }
  });
}
```

Begründung: Client-seitiges Triggern würde Polling-Latenz × Szenen-Anzahl
aufaddieren. Status-Route ist idempotent — zwei parallele Polls dürfen nicht
beide gleichzeitig enqueuen (WHERE `fal_request_ids->>'lipsync' IS NULL`
als Bedingung vor dem Submit).
...
**[Fix N2] Polling-Regeln — Batching + Idempotenz + Tab-Reopen:**

```
1. Client-Batching: ein Status-Call pro Story, nicht N Einzelcalls.
   GET /api/sceneflow/stories/[id]/status-all
     → prüft alle non-done Szenen der Story in einer Route
     → Response: SceneStatusPayload[]
   Ersetzt N parallele /scenes/[id]/status-Calls (skaliert auf 20 Szenen).

2. Idempotenz im Status-Endpoint:
   Vor R2-Upload + URL-Setzen: WHERE video_url IS NULL (oder neutral_video_url IS NULL)
   Verhindert Race bei zwei parallelen Polls:
   - Beide sehen COMPLETED
   - Beide versuchen R2-Upload → doppelter Upload
   - DB-Update: zweiter Write ist harmlos (same value), aber R2-Cost doppelt
   Lösung: SELECT ... FOR UPDATE auf der Szenen-Row vor dem Result-Fetch.
   ODER: Status-Prüfung vor dem fal.queue.result-Call:
   const fresh = await getScene(sceneId);
   if (fresh.video_url !== null) return; // bereits erledigt

3. Tab-Reopen: Beim Mount des Storyboard initial EINMAL status-all aufrufen,
   bevor setInterval startet. Verhindert bis-zu-10-s-Blindphase nach Reopen.
```

**→ File Map: Route `/api/sceneflow/stories/[id]/status-all` (GET) hinzufügen.**
...
| `app/api/sceneflow/stories/[id]/generate-images-and-voices/route.ts` | CREATE — Queue-Enqueue Phase 1 [Fix N3] |
| `app/api/sceneflow/stories/[id]/generate-videos/route.ts` | CREATE — Queue-Enqueue Phase 2 [Fix B2] |
| `app/api/sceneflow/scenes/[id]/status/route.ts` | CREATE — Polling-Endpoint, auto-enqueued Step B [Fix N1] |
| `app/api/sceneflow/stories/[id]/status-all/route.ts` | CREATE — Batch-Status für alle non-done Szenen [Fix N2] |
...
**`tests/unit/sceneflow/validation.test.ts`** — ≥ 4: [Fix W5, N4]
- speaking_character_id nicht in story.characters → 🔴
- voice_id null → 🔴
- voice_provider === 'azure' → 🔴 [Fix N4]
- image_prompt null → 🟡

**`tests/unit/api/status-all.test.ts`** — ≥ 3: [Fix N2]
- Batch-Endpoint gibt alle non-done Szenen zurück
- Idempotenz: zweiter Poll mit bereits gesetzter video_url macht keinen zweiten R2-Upload
- Tab-Reopen: initial-Fetch liefert korrekten Zustand vor erstem Intervall-Tick

**`tests/unit/api/scene-status.test.ts`** — ≥ 2: [Fix N1]
- neutral_video COMPLETED → Schritt B wird auto-enqueued (lipsync request_id gesetzt)
- bereits vorhandene lipsync request_id → kein zweites Submit (Idempotenz)
...
# Charakter mit voice_provider === 'azure' → Rote Warnung vor Start [Fix N4]
...
# "Image + Voice Generation" klicken → Queue-Enqueue, schnelle Response [Fix N3]
# Client polled /api/sceneflow/stories/[id]/status-all [Fix N2]
...
# Action-Karte: Kling-Job in DevTools → fal.queue.submit sichtbar
# Dialog-Karte: erst Kling neutral COMPLETED → status-Endpoint enqueued LipSync automatisch [Fix N1]
# Tab schließen und wieder öffnen → Initial-Fetch sofort, kein 10-s-Blindflug [Fix N2]
# Storyboard polled status-all als EINEN Call (nicht N Einzelcalls) — DevTools prüfen
...
feat(api): generate-images-and-voices — queue-enqueue Phase 1
feat(api): generate-videos — queue-enqueue Phase 2
feat(api): scenes-status — polling + auto-enqueue Schritt B
feat(api): stories-status-all — batch polling endpoint
feat(api): transfer stub + retry-image + retry-video routes
feat(sceneflow): ModelSelector dropdown
feat(sceneflow): SceneWarning — red/yellow/azure validation badges
feat(sceneflow): GenerationControls — Phase 1 + Phase 2 buttons
feat(sceneflow): ImageViewer — fullscreen + per-scene retry
feat(sceneflow): SceneCard — progress states + initial-fetch on mount
docs(limitations): Plan 8c Einträge
test: fal client + tts + scene-state + fal-to-r2 + render-pipeline + validation + status-all
```
...
Abgabe: `2026-05-25-vibegrid-plan-8c-v3-fal-render-pipeline.md`
